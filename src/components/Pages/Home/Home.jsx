import React from 'react';
import { Link } from 'react-router-dom';
import './Home.css';

const topSkills = [
  { name: 'JavaScript', logo: '/icons/icons8-javascript-96.png' },
  { name: 'React', logo: '/icons/icons8-reagir-96.png' },
  { name: 'HTML5', logo: '/icons/icons8-html-96.png' },
  { name: 'CSS3', logo: '/icons/icons8-css-96.png' },
];

function Home() {
  return (
    <section className="home-section container">

      <div className="home-content-wrapper row align-items-center">
        <div className="text-block col-md-6 text-md-start text-center order-2 order-md-1">

          <h1 className="home-title mt-4">João Andreotti</h1>

          <h2 className="home-subtitle mb-3 social-links-resumo">
            Desenvolvedor Front-end | Especialista em interfaces dinâmicas.

            <a href="https://www.linkedin.com/in/jo%C3%A3o-andreotti/" target="_blank" rel="noopener noreferrer" className="ms-3 social-icon">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
              </svg>
            </a>

            <a href="https://github.com/JoaoAndreotti90" target="_blank" rel="noopener noreferrer" className="ms-2 social-icon">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
              </svg>
            </a>
          </h2>

          <p className="home-bio mt-3 lead">
            Construção de aplicações web de alta performance e experiência otimizada para todos os dispositivos. Meu foco é traduzir designs complexos em código limpo, rápido e escalável.
          </p>

          <div className="skills-resumo-container mt-5 mb-5">
            <h3 className="resumo-title mb-3">Minha stack:</h3>
            <div className="resumo-grid d-flex justify-content-center justify-content-md-start gap-4">
              {topSkills.map((skill, index) => (
                <div key={index} className="resumo-skill-item text-center">
                  <img src={skill.logo} alt={`Logo ${skill.name}`} className="resumo-skill-logo" />
                </div>
              ))}
            </div>
          </div>


          <div className="cta-buttons d-flex gap-3 justify-content-center justify-content-md-start mt-5">
            <Link to="/projetos" className="btn btn-primary btn-lg btn-gradient">
              Ver Meus Projetos <span className="btn-icon">→</span>
            </Link>

            <Link to="/contato" className="btn btn-lg btn-outline-gradient">
              Fale Comigo
            </Link>
          </div>
        </div>

        <div className="image-block col-md-6 text-center order-1 order-md-2 mb-4 mb-md-0">
          <img
            src="/logo.jpg"
            alt="Foto de Perfil de João Andreotti"
            className="profile-pic-home shadow-lg"
          />
        </div>

      </div>
    </section>
  );
}

export default Home;